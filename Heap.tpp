...
class Heap {
    ...
    public:
        ...
        void remove(T value) {
            ...
            if(it != this->tree.end()) {
                heapIndex index = std::distance(this->tree.begin(), it);
        ...
        T getMin() {
            if(this->isHeapEmpty()) {
                return std::numeric_limits<T>::min(); // Return a minimum value if the heap is empty
            }
            return this->tree.at(1); // The root of the min-heap is at index 1
        }
};
